...
def test_ioclass_stats_basic(cache_mode: CacheMode, random_cls: CacheLineSize):
    ...
    with TestRun.step(
        ...
        for class_id in range(ioclass_config.MAX_IO_CLASS_ID + 2):
            out_of_range = (
                " out of range" if class_id > ioclass_config.MAX_IO_CLASS_ID else ""
            )

            with TestRun.group(f"Checking {out_of_range} IO class id {class_id}..."):
                expected = class_id == 0 or class_id in range(
                    min_ioclass_id, max_ioclass_id
                )

                try:
                    casadm.print_statistics(
                        cache_id=cache_id, io_class_id=class_id, io_class=True
                    )
                    if not expected:
                    ...
                    if expected:
                        TestRun.LOGGER.error(
                            f"Stats not retrieved for IO class id: {class_id}"
                        )
                    elif class_id <= ioclass_config.MAX_IO_CLASS_ID:
                        if not check_stderr_msg(
                            e.output, get_stats_ioclass_id_not_configured
                        ):
                            TestRun.LOGGER.error(
                                f"Wrong message for unused IO class id: {class_id}"
                            )
                    elif not check_stderr_msg(
                        e.output, get_stats_ioclass_id_out_of_range
                    ):
                        TestRun.LOGGER.error(
...
def test_ioclass_stats_sections(random_cls):
    ...
    with TestRun.step("Prepare cache and core devices"):
        ...
        core_device.create_partitions(
            [Size(2, Unit.GibiByte)] * cache_count * cores_per_cache
        )
    ...
    with TestRun.step(
        f"Validate displayed statistics for default IO class on each cache"
    ):
        for cache in cache_list:
            ...
            for stat_name in ioclass_stat_filter:
                ...
                if not any(
                    isinstance(statistic, io_class_stat_name)
                    for statistic in statistics
                ):
                    TestRun.LOGGER.error(
                        f"Value for {stat_name} not displayed in output"
                    )
    ...
    with TestRun.step(
        f"Validate displayed statistics for default IO class on each core"
    ):
        for cache in cache_list:
            cache_core_device_list = [
                core for core in core_list if core.cache_id == cache.cache_id
            ]
    ...
    with TestRun.step("Load random IO class configuration for each cache"):
        for cache in cache_list:
            random_list = IoClass.generate_random_ioclass_list(
    ...
    with TestRun.step(
        f"Validate displayed statistics for configured IO class on each cache"
    ):
        for cache in cache_list:
            statistics = cache.get_io_class_statistics(
                io_class_id=0, stat_filter=ioclass_stat_filter
            ...
            for stat_name in ioclass_stat_filter:
                ...
                if not any(
                    ...
                    TestRun.LOGGER.error(
                        f"Percentage for {stat_name} not displayed in output"
                    )
    ...
    with TestRun.step(
        f"Validate displayed statistics for configured IO class on each core"
    ):
        for cache in cache_list:
            cache_core_device_list = [
                core for core in core_list if core.cache_id == cache.cache_id
